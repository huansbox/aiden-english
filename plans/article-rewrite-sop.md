...
以下數據來自 5 篇 Reading Plus 課本故事（p.94-103），作為改寫目標的依據：

| 指標 | 實測範圍 | 改寫目標 | 說明 |
|------|---------|---------|------|
| 每篇字數 | 186-228 字（avg 203） | 200-250 字 | 新聞需多一點背景，略放寬 |
| 平均句長 | 7-10 字（avg 8.2） | 7-10 字 | 含 1 字感嘆句（"Splash!"）拉低平均 |
| 最長句 | 14-16 字 | 不超過 16 字 | 超過 16 字必須拆分 |
| 簡單句比例 | ~78% | ≥ 80% | SVO / SV / 疑問句 / 感嘆句為主 |
| 複合句 | ~22% | ≤ 20% | 限 when / but / and / because / as 連接 |
| 時態 | 過去簡單 74%、現在簡單 19% | 過去簡單式為主 | 對話用現在式，標題/引導問句用現在式 |
| 新詞彙 | 9-11 個/篇（Wordlist） | 5-8 個/篇 | 非虛構內容術語較多 |
| 段落數 | 5-8 段 | 5-8 段 | 每段 1-3 句 |
| 人稱 | 第一人稱 | 第一或第三人稱 | Reading Plus 用第一人稱，新聞用第三人稱 |

> 完整分析見 [Reading Plus 內容分析](reading-plus-analysis.md)
